...
import CreatTemplateUW from "./CreatTemplateUW";
...
const ManualQuery = ({ info,Id}) => {
    ...
    useJwt
      ...
      .then((res) => {
        if (res.status === 200) {
          ...
          if (element && element.classList.contains("show-composeScreen")) {
            element.classList.add("hide-composeScreen");
            element.classList.remove("show-composeScreen");
          }
        }
      })
      .catch((err) => {
  ...
  return (
    <div>
      ...
      <CreatTemplateUW toggle={open} setToggle={setOpen} />
    </div>
  );
};
