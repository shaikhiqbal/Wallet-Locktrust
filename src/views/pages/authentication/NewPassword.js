...
const [errorOTP,setErrorOTP]=useState('')
  ...
  const onSubmit = (data) => {
    ...
    useJwt.newpassword(uid, token, {password:password,confirm_password:confirm_password,otp:parseInt(otp)}).then((res) => {
      if (res.status === 202) {
        navigate('/login')
      }
    });
  ...
  const onResendOTP = () => {
    setCounter(30)
    useJwt
    .resetPasswordOtpRese(uid,token,{ uid,attempt:1,})
    .then((res) => {
      if (res.status === 200) {
  ...
  // useEffect(()=>{
  //   useJwt
  //   .newpasswordresendOtp(uid,token,{attempt:1,})
  //   .then((res) => {
  //     if (res.status === 200) {
  //       setTokenVal(res.data.token);
  //       setCounter(30)
