...
const convertNumber=(data,key)=>data[key]=0;
  const onSubmit = (data) => {
    if (applicationID == "undefined") {
      ...
      setLoader(true);
      if (transition || method || delivery) return;
      if (!data["accepted_credit_cards"]) convertNumber(data,'accepted_credit_cards')
      if(!data['monthly_sales_volume'])convertNumber(data,"monthly_sales_volume")
      if(!data["avg_transaction_value"])convertNumber(data,"avg_transaction_value")
  ...
  useEffect(() => {
    ...
    useJwt.getbusinessprofile({ application_id }).then((res) => {
      if (res?.status === 200 && res?.data[0]) {
        setDefaultData(res?.data[0]);
        setPercentState(res?.data[0]);
        reset(res.data[0]);
        setSelectedCheckboxes([])
        checkMonth(res.data[0]);
        iterate(res?.data[0])
      }
    });
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Input
                    ...
                    onChange={(e) => {
                      checkCharLength(e.target.value, 22, field.name);
                      field.onChange(e.target.value);
                    }}
                  />
                )}
